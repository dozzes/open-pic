...
void set_boundary_conditions(const std::string& lua_func_name)
{
   ...
   {
      std::string msg_fmt = "Please check function \"%s\""
                            " in \"%s\" and restart.";
...
void scatter_particle_std(const Particle& p, DensityGrid& dg)
{
   ...
   const size_t ip = static_cast<size_t>(p.r.x/h);
   const size_t jp = static_cast<size_t>(p.r.y/h);
   const size_t kp = static_cast<size_t>(p.r.z/h);

   for(size_t i = ip-1; i != ip+2; ++i)
   for(size_t j = jp-1; j != jp+2; ++j)
   for(size_t k = kp-1; k != kp+2; ++k)
   {
