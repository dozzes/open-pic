...
bool is_particle_can_scatter(Particles& particles, index_t p, const Grid& grid, const DblVector& dr)
{
    ...
    const long pi = static_cast<long>(floor(next_r.x/h));
    const long pj = static_cast<long>(floor(next_r.y/h));
    const long pk = static_cast<long>(floor(next_r.z/h));

    if (pi < 1 || pi > static_cast<long>(grid.size_x()) - 2 ||
        pj < 1 || pj > static_cast<long>(grid.size_y()) - 2 ||
        pk < 1 || pk > static_cast<long>(grid.size_z()) - 2)
    {
        particles.remove_later(p);
        return false;
    }

    const PIC::CellState home_cell_state =
        grid(static_cast<index_t>(pi),
             static_cast<index_t>(pj),
             static_cast<index_t>(pk)).state();
...
bool check_particle_move(const Particle& particle, const Grid& grid, const DblVector& dr)
{
    ...
    if (dr.abs() > h_2)
    {
        ...
        // Report the grid values at the position reached by this move, not at
        // the particle's old position.
        const DblVector next_r = particle.r + dr;
        const long pi = static_cast<long>(floor(next_r.x/h));
        const long pj = static_cast<long>(floor(next_r.y/h));
        const long pk = static_cast<long>(floor(next_r.z/h));

        const bool next_r_is_inside =
            pi >= 1 && pi <= static_cast<long>(grid.size_x()) - 2 &&
            pj >= 1 && pj <= static_cast<long>(grid.size_y()) - 2 &&
            pk >= 1 && pk <= static_cast<long>(grid.size_z()) - 2;

        Grid::NodeType point_val;
        PIC::CellState cell_state = PIC::cs_absorptive;
        if (next_r_is_inside)
        {
            PIC::from_grid_to_point(grid, next_r, point_val);
            cell_state = grid(static_cast<index_t>(pi),
                              static_cast<index_t>(pj),
                              static_cast<index_t>(pk)).state();
        }

        const std::string msg = boost::str(boost::format("Step = %1%:\n\tError CFL, particle shift dr should be  < h/2:"
                                                  "\n\tparticle's destination cell = (%2%, %3%, %4%);"
                                                  "\n\tgrid size = (%5% x %6% x %7%);"
                                                  "\n\tdr = (%8%, %9%, %10%),\n\tdr.abs() = %11%; h/2 = %12%;"
